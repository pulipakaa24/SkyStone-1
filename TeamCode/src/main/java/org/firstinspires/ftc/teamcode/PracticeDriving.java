...
public class PracticeDriving extends LinearOpMode {
    ...
    //MOTOR CONSTANTS
    static final double COUNTS_PER_MOTOR_REV = 547; //216
...
//                moveAmount += skystoneAlign();
                ...
                telemetry.addData("isFound: ", isFound);
                telemetry.update();
                mecanumEncoder(0.5, speedMod*direction, speedMod*direction, 3, "lateral");
                sleep(100);
    ...
    {
        int rLiftTarget = robot.motorLift.getCurrentPosition() + (int)(inches * COUNTS_PER_INCH);

        robot.motorLift.setTargetPosition(rLiftTarget);//assigns position
        robot.motorLift.setMode(DcMotor.RunMode.RUN_TO_POSITION);//changes to run a number of inches, rather than using encoders
        runtime.reset();//resets the runtime so that the time out works
        robot.motorLift.setPower(Math.abs(speed) + correction);//turns the motors on

        while (opModeIsActive() &&//ends the motors when time runs out
                (runtime.seconds() < timeoutS) &&
                (robot.motorLift.isBusy())){}
        robot.motorLift.setMode(DcMotor.RunMode.RUN_USING_ENCODER);//turns the motors back to encoders.
    }

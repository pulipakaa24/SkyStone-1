...
public class MecanumDriving extends LinearOpMode {
    ...
    public float skystoneAlign() //This should automatically laterally align the robot to grab the block
    {
        ...
        float moveAmount = translation.get(1) /mmPerInch - 4;//this should center the block and the grabbing mechanism. If it doesn't, try adjusting the -2
        moveAmount = moveAmount/9;
...
//            targetVisible = true; //this NEEDS to be committed out
            if (targetVisible) {
                ...
                moveAmount += skystoneAlign();
                sleep(500);
                mecanumEncoder(.75, -2, -1.5, 5, "vertical");
                sleep(500);
                robot.servoClaw.setPosition(.7);
                sleep(500);
                mecanumEncoder(0.75, 4, 2, 5, "vertical");
                ...
                telemetry.addData("isFound: ", isFound);
                telemetry.update();
                mecanumEncoder(1, speedMod*direction, speedMod*direction, 1, "lateral");
                sleep(100);
